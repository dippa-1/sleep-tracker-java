...
public class TimeSeries<Y extends Comparable<Y>> extends LineSeries {
  ...
  public void paintLine(Graphics g, Rectangle bounds) {
    ...
    if (getValues().get(0).getClass() == Integer.class) {
      ...
      int previousY = bounds.y + bounds.height
          - ((int) getValues().get(0) - minimum) * bounds.height / (maximum - minimum);
      ...
      for (int i = 1; i < getValues().size(); ++i) {
        ...
        int y = bounds.y + bounds.height
            - (int) (((float) getValues().get(i) - minimum) * bounds.height / (maximum - minimum));
        g2d.drawLine(previousX, previousY, x, y);
        previousX = x;
        previousY = y;
      }
    } else if (getValues().get(0).getClass() == LocalTime.class) {
      final ArrayList<LocalTime> values = (ArrayList<LocalTime>) getValues();
      // the following assumes that if the person goes to bed after midnight and
      // before 12:00, it is considered later than pre midnight, even though the
      // number of hours is smaller
      LocalTime minimum = values.get(0);
      LocalTime maximum = values.get(0);
      for (int i = 1; i < values.size(); ++i) {
        LocalTime currentTime = values.get(i);
        final int currentHour = currentTime.getHour();
        if (currentHour < 12) {
          if (maximum.getHour() < 12 && currentTime.isAfter(maximum) || maximum.getHour() >= 12) {
            maximum = currentTime;
          } else if (minimum.getHour() < 12 && currentTime.isBefore(minimum)) {
            minimum = currentTime;
          }
        } else if (maximum.getHour() > 12 && currentTime.isAfter(maximum)) {
          maximum = currentTime;
        } else if (currentTime.isBefore(minimum)) {
          minimum = currentTime;
        }
      }

      long maximumMinutes = (maximum.getHour()+1) * 60;
      if (maximum.getHour() < 12) {
      ...
      int previousY = bounds.y + bounds.height
          - (int) ((elementMinutes - minimumMinutes) * bounds.height / minutesRange);
      for (int i = 1; i < getValues().size(); ++i) {
        element = (LocalTime) getValues().get(i);
        elementMinutes = element.getMinute() + element.getHour() * 60;
        if (element.getHour() < 12) {
          elementMinutes += 24 * 60;
        }
        // elementMinutes = elementMinutes % (24 * 60);
        final double xProgress = calculateXProgress(startDate, totalDuration, getLabels().get(i));
        int x = (int) (bounds.x + xProgress * bounds.width);
  ...
  public void paintXAxisTicks(Graphics g, Rectangle bounds, int tickCount, int tickSize) {
    ...
    for (int i = 0; i < tickCount; ++i) {
      ...
      g2d.drawString(displayMonth + " " + displayYear, x - estimatedWidthPerMonthLabel / 2,
  ...
  public void paintYAxisTicks(Graphics g, Rectangle bounds, int tickSize, int xLabelOffset) {
    ...
    if (getValues().get(0).getClass() == Integer.class) {
      ...
      final int minHour = minimum.getHour(); // min time might actually be max time. TODO: fix this
      int maxHour = maximum.getHour() + 1;
      if (maximum.getHour() < 12) {
        maxHour += 24;
      }
